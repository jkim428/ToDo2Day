...
class DBHelper extends SQLiteOpenHelper{
    ...
    public void addTask(Task newTask)
    {
        ...
        // Everything, "except" the primary key_id (auto assigned)
        ContentValues values = new ContentValues();
        values.put(FIELD_DESCRIPTION, newTask.getDescription());
        values.put(FIELD_DONE, newTask.isDone() ? 1 : 0);
        db.insert(DATABASE_TABLE, null, values); // no explanation for null, just null :)
        db.close();
    }
    ...
    public List<Task> getAllTasks()
    {
        ...
        if (cursor.moveToFirst())
        {
            ...
            while (cursor.moveToNext()); // continue as long as cursor moves to the next
        }
        cursor.close();
        db.close();
        return allTasksList;
